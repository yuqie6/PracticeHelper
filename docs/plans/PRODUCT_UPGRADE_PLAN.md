...
| 12  | Prompt 版本管理   | P2     | 🟡       | sidecar 已支持 prompt set registry、训练页选版本、session 绑定版本、前端安全 overlay、默认偏好、实验页跨 session A/B 对比和审计明细；核心 prompt 仍只在源码 markdown 维护，不开放前端完整编辑                                |
...
3. P2 里已经收口完成的是前端体验增强、数据导出和 Prompt 版本管理第二阶段；下一步如果继续推进，更适合单开 `6.4 RAG 升级` 或更强的 Prompt 实验分析能力，而不是回头去做“前端完整编辑核心 prompt”这类已经明确不在当前边界内的需求
