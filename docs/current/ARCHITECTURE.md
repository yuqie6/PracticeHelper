...
sidecar/app/
  ...
  schemas.py         # 主请求/响应模型（generate_question / evaluate_answer / generate_review 等）
  schema_runtime.py  # runtime trace / side_effects / command_results / stream envelope
  schema_memory.py   # observation / session summary / memory 检索相关模型
  schema_common.py   # 通用枚举与共享片段
  langgraph_flows.py # 5 条 LangGraph 图：repo 分析 / JD 分析 / 出题 / 评估 / 复盘
  agent_runtime.py   # 兼容门面：re-export `runtime/` 下的 AgentRuntime
  agent_tools.py     # 兼容门面：re-export `runtime_tools/` 下的工具构造
