...
> 状态：本文已同步到 2026-03-23 当前工作区。

当前工程收口执行计划见 [ARCHITECTURE_CONVERGENCE_PLAN.md](../plans/ARCHITECTURE_CONVERGENCE_PLAN.md)；
...
server/
  ...
  internal/
    ...
    controller/internal_controller.go # 仅供 sidecar 调用的 internal 检索、session 详情与 agent command gate
    domain/
      constants.go            # 共享状态常量与枚举值
      profile.go              # 用户画像相关 DTO
      project.go              # 项目导入、项目画像与 repo chunk DTO
      job_target.go           # JD 与分析快照 DTO
      training.go             # session / turn / review / weakness / dashboard DTO
      memory.go               # observation / summary / retrieval / memory index DTO
      prompt.go               # prompt set / prompt experiment / 审计 DTO
      runtime_contracts.go    # sidecar request/response envelope、stream event、command_results 契约
      agent_command.go        # typed command envelope / result / command type 常量
    infra/sqlite/
    ...
    service/
      ...
      review_recommendation_service.go # review 推荐归一化、知识图谱兜底与 prerequisite 边
      agent_context_service.go # question / answer / review 的 agent_context 预装载
      internal_service.go     # sidecar internal 接口对应的服务逻辑
      agent_command_service.go # /internal/agent-commands 的 command 校验与 resolver
      runtime_trace_service.go # runtime_trace 归一化、裁剪与持久化辅助
    ...
    sidecar/
      client.go               # sidecar HTTP 客户端对外调用面
      decode.go               # 同步 response envelope / trace / command_results 解码
      stream.go               # NDJSON 流事件解码与最终结果聚合
      errors.go               # APIError、重试策略与 prompt meta 解析
...
sidecar/app/
  ...
  flows/
    langgraph.py     # 5 条 LangGraph 图：repo 分析 / JD 分析 / 出题 / 评估 / 复盘
  prompts/
  ...
  runtime/
    facade.py        # AgentRuntime 门面与各 flow 入口
    specs.py         # question / evaluate / review / analyze 的 task spec 组装
    engine.py        # 非流式 agent loop + fallback 编排
    streaming.py     # 流式 agent loop + status/result 事件编排
    single_shot.py   # sync / stream single-shot fallback
    validation.py    # 业务校验、tool binding 与 resolved command 语义
    trace.py         # runtime_trace 追加、command status 映射与错误码归类
    state.py         # TaskExecutionResult 与单次 run runtime state
  runtime_tools/
    prepared.py      # context compaction 与 prepared tool payload
    readers.py       # recall/search/detail 只读工具
    actions.py       # observation / knowledge / depth / next-session side effects
    commands.py      # typed command 工具、budget、幂等去重与结果序列化
    builders.py      # 各 flow 的工具组装
  schemas/
    __init__.py      # 主请求/响应模型聚合导出
    runtime.py       # runtime trace / side_effects / command_results / stream envelope
    memory.py        # observation / session summary / memory 检索相关模型
    common.py        # 通用枚举与共享片段
  shared/
    tooling.py       # RuntimeTool、参数解析与共享校验逻辑
    compaction.py    # 上下文压缩与 compaction 细节
```
...
这意味着当前 sidecar 已经不是“只读工具的受约束推理器”了，而是带长期记忆装载、行动工具和 Go 侧副作用回写的受约束 agent；但它仍然不是完全自治、可自由规划的通用 agent。当前更准确的目标，是把它继续升级成“训练域里的成熟 agent runtime”，而不是把系统直接改写成通用 agent 平台。对应的后续升级方案见 [AGENT_DEEP_REDESIGN_PLAN.md](../plans/AGENT_DEEP_REDESIGN_PLAN.md)。

### 3.2 仓库导入管线（repo_analysis/context.py）
...
5. 导入完成后如果向量写入已启用，会异步把当前项目的 `repo_chunks` 加入 embedding 队列；这一步失败不会回滚项目导入
...
- `evaluate_answer` / `generate_review` 的校验、重试和 side_effects 收口都已经下沉到 `runtime/engine.py`、`runtime/streaming.py`、`runtime/single_shot.py` 和 `runtime/validation.py`
...
2. **Context Engine**：由 Go 预装 `agent_context`，并在 sidecar 内做材料压缩、按需补查和 token budget 裁剪。当前 observations / session summaries 已走 `memory_index + embedding + optional rerank` 的混合检索；repo chunk 已升级为 `Qdrant vector recall + optional rerank + FTS5 fallback`。
3. **Planning Engine**：LangGraph 继续保持薄壳，只承担最小策略节点和步骤编排；任务内真正的决策、校验和 fallback 继续收在 runtime。
4. **Action Engine**：当前是双轨模型。cheap/local/batch-friendly 的动作继续走 `side_effects`，关键状态决策则已经有第一版 typed command path（`transition_session`、`upsert_review_path`），通过 Go internal callback `/internal/agent-commands` 返回结构化结果；Go 仍负责最终状态迁移与持久化。
5. **Validation and Recovery**：`AgentRuntime` 内部负责 JSON 校验、语义校验、loop 内重试、single-shot fallback 与 stream fallback。
6. **Observability**：Go 继续记录 `prompt_set_id`、`prompt_hash`、`model_name`、`raw_output`、`latency` 等审计字段；`runtime_trace` 已经贯穿 sidecar runtime、Go 持久化阶段和前端展示，流式链路除了 `phase / context / reasoning / result` 之外，还会输出 `trace`，并通过 `status` 暴露 `command_requested / command_applied / command_deferred / command_rejected` 这类命令状态。
...
- `sidecar/app/` 顶层只保留 `main.py`、`config.py` 和必要的 `__init__.py`；不要再把 adapter、prompt、schema、repo analysis、flow 或 shared helper 平铺回顶层。
- 外部适配器只能进入 `sidecar/app/adapters/*`；prompt 模板、加载和动态拼装只能进入 `sidecar/app/prompts/*`；schema 只能进入 `sidecar/app/schemas/*`；仓库分析逻辑只能进入 `sidecar/app/repo_analysis/*`；LangGraph 图层只能进入 `sidecar/app/flows/*`；共享辅助逻辑只能进入 `sidecar/app/shared/*`。
- 新的 runtime 逻辑只能进入 `sidecar/app/runtime/*`。
- 新的工具构造、tool binding、command tool 或 side-effect tool 逻辑只能进入 `sidecar/app/runtime_tools/*`。
- 不再保留顶层兼容门面；如果需要统一对外导出，应该在包内 `__init__.py` 处理，不要重新引入顶层 facade 文件。
...
- 事件类型目前包括：
  - `status`：Go 侧阶段状态事件（如 `evaluation_started` / `review_saved`），也承载 typed command 的命令状态（如 `command_requested` / `command_applied` / `command_deferred` / `command_rejected`）
  - `phase`：阶段切换
  - `context`：已读取的上下文类型
  - `trace`：结构化运行轨迹，覆盖工具调用、`command` phase、校验、fallback、persist 等阶段
  - `reasoning`：可公开的推理摘要
  - `content`：模型输出片段
  - `result`：最终结构化结果
  - `error`：流式过程中出现的错误
- 当 `status` 事件来自 typed command path 时，`data` 至少包含 `command_type`、`command_id` 和 `idempotency_key`，方便 Go 审计和前端 trace 面板对齐。
- 这里展示的是“可公开推理摘要”，不是模型原始隐藏思维链。
- 前端不会再直接把 `content` 当原始 JSON 文本整段展示，而是按任务片段拆分，再尽量解析成“问题草稿 / 评估结果 / 复盘草稿”三类结构化卡片。
- 当前流式链路也优先走 agent loop，把工具读取过程和命令回调过程转成 `phase / context / status / reasoning / trace` 事件；只有 agent loop 无法稳定收口时，才退回 single-shot streaming。
- 流式 `result` 事件内部已经可以携带 `raw_output`、`side_effects`、`command_results` 和 `trace`，供 Go 侧统一解析并落库。

## 3.7 智能体任务环境（PEAS）

这里的“智能体”特指 sidecar 里的 `AgentRuntime`。真正承担“分析仓库、出题、评分、追问、复盘”任务的实现已经收在 `sidecar/app/runtime/`；`server/` 负责编排、状态管理和持久化，`web/` 负责交互展示。
...
SQLite 当前共 23 张表/虚拟表（其中 22 张普通表 + 1 张 FTS5 虚拟表），启动时由
...
| `repo_chunk_embedding_records` | repo chunk 向量索引元数据 | repo_chunk_id（UNIQUE）, project_id, content_hash, model_name, vector_store_id, vector_dim, status |
| `repo_chunk_embedding_jobs` | repo chunk 向量回填 / 重试队列 | repo_chunk_id（UNIQUE）, project_id, status, attempt_count, claim_token, error_message |
...
| `evaluation_logs` | 生成/评估耗时审计 | session_id, turn_id, flow_name, model_name, prompt_set_id, prompt_hash, raw_output, runtime_trace_json, latency_ms |
...
| `agent_observations` | 模型沉淀的 pattern / misconception / growth / strategy note | session_id, fingerprint, scope_type, scope_id, topic, category, content, tags_json, relevance |
...
| `memory_embedding_records` | 长期记忆向量化结果 | ref_table, ref_id, embedding_json, dimensions, model_name, updated_at |
| `memory_embedding_jobs` | embedding 后台补算任务 | ref_table, ref_id, status, claim_token, claim_expires_at, error_message |
...
3. sidecar 评估完成后，Go 会先读取结构化结果，再决定下一步：
   - 常规 cheap/local 动作继续吃 `side_effects`
   - 如果 sidecar 发起了 `transition_session` command，Go 会先解析 command result，再决定是否 `skip_followup` / `extend`
   - 没有 command result 时，才按 `side_effects.depth_signal` 和当前 `turn_index / max_turns` 正常判断
   - 当前优先级固定为：`command_results > side_effects.depth_signal > 默认 turn/max_turns 判断`
4. 如果最终判断还需要继续追问：
   - 保存本轮回答和评估结果
   - 从评估结果里取下一轮问题
...
- sidecar 可以通过 `recommended_next`、`side_effects.recommended_next` 或 `upsert_review_path` command draft 给出下一轮建议
- Go 在 `persistReview` 时会继续吃 `upsert_review_path` 的结构化结果并做最终归一化，确保 `mode / topic / project_id / reason` 至少满足当前 session 语义
- 推荐收口优先级固定为：`upsert_review_path command result > recommended_next / side_effects.recommended_next > normalize fallback`
...
另外，sidecar 还会走一组 **仅内部使用** 的 `/internal/*` callback 接口；它们不属于对前端公开的 REST 契约，但属于当前 runtime 主链路的一部分。
...
### sidecar internal callback（非公开）

| 方法 | 路径 | 用途 |
|------|------|------|
| GET | `/internal/search-chunks` | sidecar 按需回调项目 chunk 检索 |
| GET | `/internal/session-detail/:id` | sidecar 获取 session + review 详情 |
| POST | `/internal/agent-commands` | sidecar 提交结构化 command，由 Go 返回 FSM / review path 的规范化结果 |
...
项目训练时的 repo chunk 检索现在走“向量主召回 + FTS5 兜底”：

- 检索范围限于当前项目的 `repo_chunks`
- 查询词由项目的 `followup_points` + `summary` 拼接而成
- 先对 query 做 embedding，在 Qdrant 中按 `document_kind=repo_chunk + project_id` 做向量召回
- 启用时只对 topK 候选做 rerank；rerank 失败不会中断主链路
- embedding、Qdrant 查询、回源或候选不足时，回退到 SQLite FTS5 检索
- FTS5 查询构建：过滤掉 < 2 字符的 token，每个 token 加双引号后用 `OR` 连接
- 无 FTS 匹配时再回退到按 `importance DESC` 排序取 top N
- 每次检索默认取 6 个 chunk
- 新导入项目会自动入队建立 repo chunk 向量索引；旧项目在服务启动后会后台补齐缺失索引
...
- **SQLite 是系统真相源，Qdrant 只存可重建索引**：业务状态、审计和训练数据仍在 SQLite；Qdrant 只负责 repo chunk / memory 的向量召回。
...
- **SQLite 是系统真相源，Qdrant 只存可重建索引**：就算向量索引丢失，也可以从 SQLite 里的 repo chunk / memory 材料重新回填，不会污染业务状态。
