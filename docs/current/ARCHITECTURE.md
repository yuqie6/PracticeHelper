...
6. **Observability**：Go 继续记录 `prompt_set_id`、`prompt_hash`、`model_name`、`raw_output`、`latency` 等审计字段；`runtime_trace` 已经贯穿 sidecar runtime、Go 持久化阶段和前端展示，流式链路除了 `phase/context/reasoning/result` 之外还会输出 `trace`。
...
- 事件类型目前包括：
  - `status`：Go 侧阶段状态事件（如 `evaluation_started` / `review_saved`）
  - `phase`：阶段切换
  - `context`：已读取的上下文类型
  - `trace`：结构化运行轨迹，覆盖工具调用、校验、fallback、persist 等阶段
  - `reasoning`：可公开的推理摘要
  - `content`：模型输出片段
  - `result`：最终结构化结果
  - `error`：流式过程中出现的错误
...
- 当前流式链路也优先走 agent loop，把工具读取过程转成 `phase/context/reasoning/trace` 事件；只有 agent loop 无法稳定收口时，才退回 single-shot streaming。
