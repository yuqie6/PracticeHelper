...
SQLite 当前共 15 张表（含 1 张 FTS5 虚拟表），启动时由 `internal/infra/sqlite.Bootstrap`
自动建表并补种子数据：
...
一次训练会话现在是**可配置多轮**，默认 2 轮，最多 5 轮。

状态流转不再是固定的“主问题 + 1 次追问”，而是循环到 `max_turns` 为止：

```
waiting_answer ──用户回答当前轮──▶ evaluating ──未到 max_turns──▶ waiting_answer
                                                └──已到 max_turns──▶ review_pending ──生成复盘卡──▶ completed
```
...
5. `RetrySessionReview`：只有 `review_pending` 会话允许重试。服务端会先把状态原子切到 `evaluating`，避免重复点“重试生成复盘”触发并发复盘。

这里有两个实现细节要明确：

- 追问已经不是内嵌字段，而是独立 `turn`
- 多轮训练的轮次判断完全由 `turn_index` 和 `max_turns` 决定
