...
| 5   | LangGraph 多节点  | P1     | ✅       | `analyze_repo` 已补 `collect_bundle -> rank_chunks -> summarize`，`evaluate_answer` 已补 `retrieve_context -> evaluate -> validate_output -> re_evaluate`，`generate_review` 也已补校验与重试                 |
...
- `generate_review` 已增加 `prepare_review_context -> generate_review -> validate_review -> re_generate`
- sidecar 内部返回已经统一收成 envelope，图节点重试后仍能保留最终 `raw_output`

当前实现说明：

- 当前已经覆盖产品主链路里最关键的仓库分析、出题、评估和复盘节点拆分
...
3. P2 里已经收口完成的是前端体验增强、数据导出和 Prompt 版本管理 v1；下一步如果继续推进，更适合单开 `6.4 RAG 升级` 或更强的 Prompt 实验分析能力，而不是回头重复补 P1
