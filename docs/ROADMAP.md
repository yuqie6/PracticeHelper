...
PracticeHelper 的 v0 目标是：一个面向后端 / Agent 求职方向的单用户面试训练 Agent。v0 之后的产品方向与演化阶段见 [VISION.md](./VISION.md)。
...
v1 候选的优先级和阶段划分见 [VISION.md](./VISION.md) 的演化路线。上面的条目不是并列关系——JD 定制训练是阶段 B 的核心，简历联动当前不在近期计划内。
...
v0 之后的开发计划以 `docs/PLAN.md` 为准，产品方向以 `docs/VISION.md` 为准。
