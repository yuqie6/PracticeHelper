...
PracticeHelper 由 **三个本地进程** 通过 HTTP 通信组成：
...
| **存储** | 全部数据存一个 SQLite 文件 | 当前 23 张表/虚拟表（含 1 张 FTS5 虚拟表） |

### 关键设计决策

- **单用户单体架构** —— 先让系统对一个人真正有用，验证训练价值后再考虑多用户
- **SQLite 即全部存储** —— 画像、项目、会话、弱项、知识图谱全在一个文件里，零基础设施依赖
...
<details>
<summary>可选：GitHub Token 与超时调优</summary>
